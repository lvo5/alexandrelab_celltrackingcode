...
function [lub] = trackmem(xyzs,maxdisp,param)
tic
dd = length(xyzs(1,:));                

if nargin==2
    %default values
    memory=0; % if mem is not needed set to zero
    goodenough = 30;  % if goodenough is not wanted set to zero
    dim = dd - 4;
    quiet=0;
else
      memory    =   param.mem;
...
% Only consider X, Y positions, angle, time and Track ID
lub(:,3:4) = [];
end
%done=toc,StartTime
