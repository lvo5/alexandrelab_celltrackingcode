...
% 
%   The intensity weighted centroid surrounding the brightest pixel serves
%   as a refinement for the particle location and provides sub-pixel resolution
...
%  for matlab
%   
%           Particle location refinement based on calculating the
%           centroid of the intensity distribution around the central 
%           bright pixel
% 
% INPUT:
...
[nr,nc]=size(im);
% IMPORTANT ===> CELL CLOSE TO EDGES
%remove all potential locations within distance sz from edges of image
ind=find(mx(:,2) > 0.5*sz & mx(:,2) < nr-0.5*sz);
mx=mx(ind,:);
ind=find(mx(:,1) > 0.5*sz & mx(:,1) < nc-0.5*sz);
mx=mx(ind,:);
